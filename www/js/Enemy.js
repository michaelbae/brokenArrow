(function (window) {
	...
	p.getEnemy = function (type) {
		if (type == BASIC_ENEMY) {
			...
			this.points = BASIC_ENEMY_POINTS;
		...
		if (type == SPIRITUAL_ENEMY) {
			...
			this.points = SPIRITUAL_ENEMY_POINTS;
		...
		if (type == SHOOTER_ENEMY) {
			...
			this.points = SHOOTER_ENEMY_POINTS;
		...
		if (type == CHARGER_ENEMY) {
			...
			this.points = CHARGER_ENEMY_POINTS;
		...
		if (type == EXPLOSION_ENEMY) {
			...
			this.type = type;
			this.skill_stack = 1;
			this.points = EXPLOSION_ENEMY_POINTS;

			//draw Enemy
			var animation = BOMB_ENEMY_ANIMATION.clone();
			animation.gotoAndPlay("run");
		...
		if (type == LASER_ENEMY) {
			...
			this.skill_stack = 20 + this.randomNumber(0, 100);
			this.points = LASER_ENEMY_POINTS;
		...
		if (type == SPAWN_ON_DEATH_ENEMY) {
			...
			this.points = SPAWN_ON_DEATH_ENEMY_POINTS;
		...
		if (type == SHIELDING_ENEMY) {
			...
			this.skill_stack = 1;
			this.points = SHIELDING_ENEMY_POINTS;

			var animation = SHIELD_ENEMY_ANIMATION.clone();
			animation.gotoAndPlay("run");
			//draw Enemy
			this.enemy = animation;
			this.addChild(this.enemy);
		...
		if (type == BLAZER_ENEMY) {
			...
			this.skill_stack = 4;
			this.points = BLAZER_ENEMY_POINTS;
			//draw Enemy
			this.enemy = new createjs.Shape();
		...
		if (type == JUMPING_ENEMY) {
			...
			this.skill_stack = 1;
			this.points = JUMPING_ENEMY_POINTS;
			//draw Enemy
			this.enemy = new createjs.Shape();
