...
function setStage(stageLevel) {
	...
	switch (stageLevel) {
		//(stageLevel, enemyTypes, enemyRatios, enemiesLimit, startingEnemys, mapWidth, mapHeight, objective, objectiveNum, spawnIntervalMin, spawnIntervalMax)
		case 1:	
			constructStage(stageLevel, [5], [1], 8, 3, 800, 500, 1, 15, 2, 3);
		break;

		case 2:	
		//make spawning speed configurable
			constructStage(stageLevel, [2, 1], [0.1, 0.9], 20, 10, 1000, 600, 1, 12, 1, 2);
		break;

		case 3:	
			constructStage(stageLevel, [2, 1, 4], [0.5, 0.3, 0.2], 20, 10, 800, 500, 2, 800, 2, 3);
		break;

		case 4:	
			constructStage(stageLevel, [2, 3, 5], [0.6, 0.3, 0.1], 20, 30, 2000, 2000, 1, 30, 1, 4);
		break;

		case 7:	
			constructStage(stageLevel, [3, 4, 5], [0.3, 0.3, 0.4], 20, 30, 1000, 600, 1, 30, 0.1, 10);
		break;

		case 8:	
			constructStage(stageLevel, [3, 9, 7], [0.6, 0.3, 0.1], 15, 40, 1000, 800, 2, 600, 4, 4);
		break;

		case 9:	
			constructStage(stageLevel, [7, 10, 6], [0.6, 0.3, 0.1], 15, 30, 900, 900, 1, 30, 3, 3);
		break;
...
*/
function constructStage(stageLevel, enemyTypes, enemyRatios, enemiesLimit, startingEnemys, mapWidth, mapHeight, objective, objectiveNum, spawnIntervalMin, spawnIntervalMax) {
	spawnIntMin = spawnIntervalMin;
	spawnIntMax = spawnIntervalMax;
	enemyCountDown = randomNumber(spawnIntervalMin * 10, spawnIntervalMax * 10); 
...
function handleTick(event) {
	if (!event.paused) {
		...
		bulletDisplayTick();
		stageTime++;
		if (enemyCountDown <= 0 && spawnEnemyTickEnabled) {
			spawnEnemyTick(spawnIntMin, spawnIntMax);
			console.log("reset");
		}
			console.log(enemyCountDown);
		enemyCountDown--;
	}
}
...
function spawnEnemyAt(chosenType, x, y) {
	...
	if ((enemies.length < totalEnemy) || enemy.type == 8) {
...
function updateEnemies(){
	...
	for (var i = 0; i < enemies.length; i++){
		...
		if (!enemy.alive) {
			dead.push(i);
		} else if (!isNaN(enemy.getMatrix().tx)) {
			updateEnemy(enemy);
		} else {
			dead.push(i);
		}
...
function spawnEnemyTick(randMin,randMax){
	enemyCountDown = randomNumber(randMin * 10, randMax * 10); 
	console.log('chaining: ' + enemyCountDown);
	...
	if (num == 0) {
		var enemy = spawnEnemyAt(currentSpawnEnemy, stageMap.x, y);
	} else if( num == 1) {
		var enemy = spawnEnemyAt(currentSpawnEnemy, x, stageMap.y);
	} else if (num == 2) {
		var enemy = spawnEnemyAt(currentSpawnEnemy, stageMap.x + stageMapWidth, y);
	} else if (num == 3) {
		var enemy = spawnEnemyAt(currentSpawnEnemy, x, stageMap.y + stageMapHeight);
	}
}
